 ...
 */
public class Test02 {
	public Test02() {//기본 생성자(생성자 함수), 생성자 함수는 반환값 타입을 기술하면 안 된다.
		//abc();
		...
		//출력하고
		System.out.print("1. 정수 두 개 입력 : ");
		toPrint(no1, no2); 
		
		System.out.print("2. 정수 세 개 입력 : ");
	...
	public int calc(int no1, int no2) {
		//매개변수는 여러 개가 입력되면 , 단위로 나열한다.
		
		//반환값 변수
		int result = 0;
		
		//이 함수를 실행할 때 입력된 두 정수는 no1, no2 변수에 기억될 것이다
		//이 변수에 기억된 데이터를 계산하면 된다.
		result = no1 + no2;
		
		//반환값 내보내고
		return result; // 1. 실행 중인 이 함수를 즉시 종료, 2. 결과값이 있으면 결과값을 내보내고 함수 종료
	}
	...
	public void toPrint(int no1, int no2) {
		...
		*/
		...
		return; // 기술하지 않아도 실행내용을 모두 실행했으므로 이 함수가 자동 종료된다. 생략해도 같은 효과
	}
	
	// 두 수와 결과를 입력해주면 계산 결과를 출력해주는 함수
	public String toPrint(int no1, int no2, int no3) {
		// 매개변수 리스트에 정의된 변수는 이 함수 블럭에서만 사용할 수 있는 지역변수 
		
		/* 함수의 오버로딩 (함수의 중첩) : 같은 이름으로 함수를 한 클래스 내에 추가하는 방법
		 	<원칙> 

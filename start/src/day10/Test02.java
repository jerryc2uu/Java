package day10;
/*
   블랙핑크 친구들의 정보를 기억시켜서 출력하라
 */
public class Test02 {
	public Test02() {
		// 제니 객체
		Friend f1 = new Friend();
		//f1.name = "제니"; // 이 변수는 접근지정자가 private이므로 다른 클래스에서 접근 불가
		// 제니 정보 셋팅
		f1.setName("제니");
		f1.setAddr("ygM");
		...
		*/
		...
		System.out.println(f1);//변수만 쓰면 toString 함수 자동 호출
		
		// 로제 객체
		Friend f2 = new Friend();//Friend 타입
		...
		// 4 멤버를 하나의 변수로 관리
		Friend[] black = new Friend[4]; // Friend 타입의 데이터 기억할 빈 방만 확보해놓은 상태, 각 공간에는 데이터 없음(null)
										// 모든 클래스는 배열로 쓸 수 있다.
		//Friend tmp1 = black[0];
		
		/* System.out.println("name : " + tmp1.getName());
		 *  
		 * 아직 Friend의 객체가 각 방에 채워지지 않았다. 따라서 멤버 함수를 호출할 수 없다.
		 */
		// 위에서 만든 Friend 객체를 순서대로 배열에 채워넣는다
		black[0] = f1;
		black[1] = f2;
		black[2] = f3;
		black[3] = f4;

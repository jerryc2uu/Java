 ...
 */
public class Test03 { // 기본형 데이터 타입은 자동 초기화, 참조형 타입은 null로
	String[] names; // 변수만 만든 상태, 공간이나 데이터는 전혀 없음(초기화 안 함)
	String[] tels;
	String[] email;
	String[] birthday;
	int[] naee;
	...
	public Test03() {
		// 멤버 변수들 초기화
		setData();//함수는 반드시 호출되어야만 내용물이 실행된다. // -> 필요한 데이터들 준비된 상태
	...
	public void toPrint(Friend[] b) {
		for(int i = 0 ; i < b.length ; i++) {
			//이때 배열의 각 방에 기억된 객체들의 멤버변수는 접근이 불가능하므로 접근할 수 있는 함수를 호출해서 처리해야 한다.
	...
	public Friend[] getBlack() {
		...
		Friend[] anne = new Friend[4];//Friend 타입의 객체 4개를 담을 빈 방만 준비한 상태
		
		//각 방에 Friend 객체들을 채워넣는다. 참조형이기 때문에
		for(int i = 0 ; i < 4 ; i++) {
			//4명분 데이터가 입력되어야 하므로 Friend 객체를 4개 만들어서 각 방에 집어넣는다.
			anne[i] = new Friend();
		}
		//위 반복문이 종료되면 각 방에 있는 객체의 변수에 접근해서 데이터를 입력할 수 있는 상태가 된다.
		//데이터를 채워서
	...
	public void setData() {
		...
		tels = new String[] {"010-1111-1111", "0101-2222-2222", "010-3333-3333", "010-1234-5678"};
		email = new String[] {"jennie@githrd.com", "lisa@githrd.com", "rose@githrd.com", "jisso@githrd.com"};

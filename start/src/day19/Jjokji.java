...
public class Jjokji {
	...
	/*
	   전자는 쪽지를 보내기 위한 소켓, 후자는 쪽지를 받기 위한 소켓
	   
	   반드시 두 개를 만들어야 하는 건 아니고 소켓 하나만 가지고 있어도 보내기와 받기를 할 수 있다.
	   다만 하나를 공유하는 경우 보내는 도중에는 받지를 못할 수 있다.
	   따라서 안전한 통신을 위해 보내는 소켓과 받는 소켓을 구분해서 처리하는 게 좋다.
	...
	public Jjokji() {
		...
		frame.setResizable(false);//사이즈 조절 안 됨
		
		/*
		   이제 프로그램이 시작되었으므로
		   네트워크 구성을 할 차례
		   여기서는 UDP통신을 해야 하므로 
		   DatagramSocket을 이용해서 네트워크 구성을 해야 한다.
		 */
		...
		try {
			...
			//쪽지를 보내고 받을 수 있게 되었다.
			//받는 프로그램을 시작하자.
			//받는 프로그램은 다른 프로그램들과 병행해서 처리가 돼야 하므로 스레드로 구성
			
			//New Born
			RecieveThread t = new RecieveThread(this);
			
			//Runnable 상태로 전위
			t.start();
		} catch(Exception e) {
			//위 작업이 예외 발생한 것은 이 프로그램이 더이상 제 기능을 할 수 없는 상태이므로
			//열려있는 소켓을 모두 닫아준다.
			close();
			e.printStackTrace();
		} 
